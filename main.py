...
def runSim(robot_type=RobotType.AM, planner_type=PlannerType.RAMP, controller_type=ControllerType.PVFC, sim_time=10, dt=0.01, plot_passivity=True):
    # create modules
    ...
    return robot, planner, controller, ts, us, Fs, F_rs, f_es, qs, q_dots, q_r_dots, Vs

def getTaskSpaceState(q, q_dot, tool_length):
    q_T, q_T_dot = zip(*[util.configToTask(q_i, q_dot_i, tool_length) for q_i, q_dot_i in zip(np.concatenate(q,axis=1).T[:,:,None], np.concatenate(q_dot,axis=1).T[:,:,None])])
    q_Ts, q_T_dots = np.concatenate(q_T,axis=1), np.concatenate(q_T_dot,axis=1)
    return q_Ts, q_T_dots
    
    # if bot != 'quad':  # get task space variables
...
def plotResults(planner, controller, robot, ts, us, Fs, F_rs, f_es, qs, q_dots, q_r_dots, Vs):
    ...
    if planner.__class__.name__=='UpRampVelocityField': plotter.plotRamp(fig, ax, q_Ts, color='black')
...
def plotAugmentedResults(planner, controller, robot, ts, us, Fs, F_rs, f_es, qs, q_dots, q_r_dots, Vs, *args):
    vel_tracking_fig, vel_tracking_ax = args
    #  sim visualization
    ...
    fig, ax = plt.subplots(2, 1, figsize=(16,9), sharex=True)
    plotter.plotVelocityTracking(vel_tracking_fig, vel_tracking_ax, ts, q_T_dots, q_r_dots, Vs)
...
if __name__ == '__main__':
    ...
    runSim(robot_type=robot_type, planner_type=planner_type, controller_type=controller_type, sim_time=60)  
    # runVizVelocityField()
