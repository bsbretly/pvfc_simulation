...
def check_module_compatiblity(robot_type, planner_type):
    if robot_type==RobotType.QUAD and planner_type==PlannerType.RAMP: 
...
def create_robot(robot: RobotType) -> Robot:
    robots = {
...
def create_planner(plan: PlannerType, robot: RobotType) -> VelocityPlanner:
    base_robot_planners = {
...
def create_controller(controller: ControllerType, robot_params: namedtuple) -> BaseControl:
    controllers = {
...
def run_sim(robot_type=RobotType.AM, planner_type=PlannerType.RAMP, controller_type=ControllerType.PVFC, sim_time=10, dt=0.01, plot=True):
    # create modules
...
def plot_sim_summary(plotter, planner_type, ts, us, qs):
    ...
    if planner_type==PlannerType.RAMP.value:
