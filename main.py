...
def runAMSim(sim_time=10, dt=0.01):
    ...
    planner = PlanarVelocityField(params.BaseAMPlannerParams(), params.PlanarPlannerParams())
    ...
    q_T, q_T_dot = zip(*[util.configToTask(q_i, q_dot_i, robot.dynamics.tool_length) for q_i, q_dot_i in zip(np.concatenate(q,axis=1).T[:,:,None], np.concatenate(q_dot,axis=1).T
    [:,:,None])])
    ...
    plotter = PlotAMSimResults(planner, controller, robot,ts, us, Fs, f_es, qs, q_dots, q_Ts, q_T_dots, q_r_dots, V_Ts)
