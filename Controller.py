...
class BaseControl:
    def __init__(self, attitude_control_params, dynamics):
...
class PVFC(BaseControl):
    def __init__(self, robot_parameters, attitude_control_params, pvfc_params):
        if type(robot_parameters).__name__ == 'quadrotor_params': dynamics = QuadrotorTranslationalDynamics(robot_parameters)
        elif type(robot_parameters).__name__ == 'AM_params': dynamics = AerialManipulatorTaskDynamics(robot_parameters)
        else: raise NotImplementedError("Robot parameters not implemented for the controller.")
        super().__init__(attitude_control_params, dynamics)
