...
class PlotSimResults:
    ...
    def plotPowerAnnihilation(self, fig, ax):
        f_power =  np.array([self.q_T_dots[:,i].reshape(-1,1).T@self.Fs[:2,i].reshape(-1,1) for i in range(self.q_T_dots.shape[1])]).squeeze()
        f_r_power = self.q_r_dots*self.Fs[-1,:]

        ax[0].plot(self.ts, f_r_power, label=r'$x_r f_r$')
        ax[0].plot(self.ts, f_power, label=r'$\dot{\boldsymbol{x}}^T \tilde{\boldsymbol{F}}$')
        ax[0].legend()
        ax[0].set_ylabel(r'$[W]$')

        ax[1].plot(self.ts, f_power + f_r_power, linestyle=(0,(1,1)), label=r'$\dot{\boldsymbol{x}}^T \tilde{\boldsymbol{F}} + x_r f_r$')
        ax[1].legend()
        ax[1].set_ylim(-0.02, 0.02)
        ax[1].set_ylabel(r'$[W]$')
        ax[1].set_xlabel(r'$t\ [s]$')
        ax[1].set_xbound(0, self.ts[-1])
        return fig, ax
    
    def plotVelocityField(self, fig, ax, qs):
        min = np.amin(np.concatenate((qs[0][0,:], qs[0][1,:])))
        max = np.amax(np.concatenate((qs[0][0,:], qs[0][1,:])))
        grid = np.arange(min, max, 0.1)
        x_min = np.amin(qs[0][0,:])
        x_max = np.amax(qs[0][0,:])
        z_min = np.amin(qs[0][1,:])
        z_max = np.amax(qs[0][1,:])
    ...
    def plotRobot(self, fig, ax, ts, qs, us):
        ...
        for i in idxs:
            for j in range(len(qs)):
                self.display(fig, ax, qs[j][:,i], us[j][:,i])
        quad = mlines.Line2D([], [], color='red', marker='o', linestyle='-', markersize=5, label='quadrotor')
...
class TestVelocityField():
    def __init__(self, planarPlanner, rampPlanner):
        self.planarPlanner, self.rampPlanner = planarPlanner, rampPlanner
